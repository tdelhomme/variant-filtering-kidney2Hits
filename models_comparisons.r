...
#pdf("~/Documents/Models/variant-filtering/tests/model_comp.pdf",8,5)

for(type in list("snv",c("ins","del"))){
  ...
  train_table = train_table[which(train_table$TYPE_INFO %in% type),]
  ...
  res = foreach(i=1:10, .combine=cbind, .packages = c("randomForest","monmlp","e1071","ROCR")) %dopar% {
    ...
    model_mlp <- monmlp.fit(x, y, hidden1=5, n.ensemble=10, monotone=1, bag=TRUE)
  ...
  stopCluster(cl)
  if(!type[1]=="snv") type="indels"
  #boxplot(data.frame("RF"=res[1,], "SVM"=res[2,], "MLP"=res[3,]), col="lightgrey", ylim=c(0.95,1), main=paste("AUC 10-fold - ",type,sep=""))
  d = data.frame(auc=c(res[1,], res[2,], res[3,]), algorithm=c(rep("RF",10),rep("SVM",10),rep("MLP",10)))
  ggplot(d, aes(x=algorithm, y=auc)) + geom_violin(fill="lightgrey") + theme_bw() + ggtitle(paste("AUC 10-fold - ",type,sep=""))
}
